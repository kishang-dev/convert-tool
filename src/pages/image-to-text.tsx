...
export default function ImageToText() {
  ...
  const handleRunOcr = async () => {
    ...
    setProcessing(true);
    try {
      const result = await ocrApi.imageToText(file);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--bg)] text-[var(--text)]">
      <SEO title="Image to Text OCR - Extract Text from Photos & Scans" description="Extract editable plain text from photos and document scans." />
      <Navbar />
      ...
      <main className="flex-1 max-w-6xl mx-auto px-4 py-8 w-full">
        ...
        <div className="text-center my-8">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-pink-500/10 text-pink-500 font-semibold text-xs mb-4">
            <LuScanText size={14} /> Image OCR Engine
          </div>
          ...
          <p className="text-[var(--text-muted)] text-sm md:text-base max-w-2xl mx-auto">
            Recognize and extract editable plain text from uploaded photos and scanned documents.
          </p>
        </div>
          ...
          <div className="md:col-span-6">
            {!file ? (
              <div className="border-2 border-dashed border-[var(--border)] hover:border-pink-500 transition-colors p-8 text-center rounded-2xl bg-[var(--surface)]">
                <input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" id="ocr-img-upload" />
                <label htmlFor="ocr-img-upload" className="cursor-pointer flex flex-col items-center gap-3">
                  ...
                  <span className="font-bold text-sm">Upload Photo / Scan</span>
              ...
              <div className="p-6 rounded-2xl bg-[var(--surface)] border border-[var(--border)] shadow-sm space-y-4">
                ...
                <Button onClick={handleRunOcr} disabled={processing} className="w-full bg-pink-600 hover:bg-pink-700 text-white font-bold py-3.5 rounded-xl">
                  {processing ? "Recognizing Text..." : "Extract Text (OCR)"}
                </Button>
              </div>
            )}
          </div>
              ...
              <div>
                ...
                {extractedText !== null ? (
                  ...
                  <div className="border border-dashed border-[var(--border)] rounded-xl p-16 text-center text-xs text-[var(--text-muted)]">
                    Upload an image and click 'Extract Text (OCR)'.
                  </div>
                )}
              </div>
            </div>
        ...
        <ToolSEOContent
          ...
          toolDescription="Extract text from document scans using high-accuracy OCR."
          steps={[
            { name: "Upload Image", text: "Select your image file." },
            { name: "Run OCR", text: "Click 'Extract Text (OCR)'." },
            { name: "Copy", text: "Copy extracted text to clipboard." }
          ]}
        />
      </main>
