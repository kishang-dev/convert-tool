...
export default function MarkdownEditor() {
    const [markdown, setMarkdown] = useState(`# QuickPDF Markdown Document
...
Feel free to paste your own *.md* files here!`);
    ...
    const parseMarkdownToHtml = (md: string): string => {
        ...
        lines.forEach((line) => {
            ...
            if (isRow) {
                ...
                if (!inTable) {
                    inTable = true;
                    processedLines.push('<table class="min-w-full border border-white/10 rounded-xl my-4 text-xs font-mono">');
                    processedLines.push('<thead>');
                    processedLines.push('<tr class="bg-white/5 border-b border-white/10">');
                    cells.forEach(cell => processedLines.push(`<th class="p-2.5 text-left font-bold text-gray-300">${cell}</th>`));
                    processedLines.push('</tr>');
                    processedLines.push('</thead>');
                    processedLines.push('<tbody>');
                    tableHeaderParsed = true;
                } else {
                    processedLines.push('<tr class="border-b border-white/5 hover:bg-white/5 transition-all">');
                    cells.forEach(cell => processedLines.push(`<td class="p-2.5 text-gray-400">${cell}</td>`));
                    processedLines.push('</tr>');
                }
            } else {
                if (inTable) {
        ...
        // Lists
        html = html.replace(/^\s*-\s+(.*$)/gim, '<li class="list-disc list-inside text-gray-400 ml-4 mb-1.5">$1</li>');
        html = html.replace(/^\s*\*\s+(.*$)/gim, '<li class="list-disc list-inside text-gray-400 ml-4 mb-1.5">$1</li>');
        ...
        html = splitParas.map(para => {
            ...
            return `<p class="text-sm text-gray-300 leading-relaxed mb-4">${p}</p>`;
        }).join('\n');
    ...
    return (
        <div className="min-h-screen bg-[#0f172a] text-white">
            <Head>
                <title>Markdown Editor & HTML Converter | QuickPDF Tools</title>
            ...
            <div className="max-w-7xl mx-auto px-4 py-24">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6 bg-white/5 border border-white/10 p-4 rounded-2xl">
                    <div className="flex gap-2 flex-wrap items-center">
                        <span className="text-xs text-gray-400 font-bold uppercase mr-1">Insert:</span>
                        <button onClick={() => insertTemplate('## New Heading')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5">H2 Heading</button>
                        <button onClick={() => insertTemplate('**Bold Text**')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5 font-bold">Bold</button>
                        <button onClick={() => insertTemplate('*Italic Text*')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5 italic">Italic</button>
                        <button onClick={() => insertTemplate('`inline_code`')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5 font-mono">Code</button>
                        <button onClick={() => insertTemplate('| Header A | Header B |\n| :--- | :--- |\n| Cell 1 | Cell 2 |')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5">Table</button>
                        <button onClick={() => insertTemplate('- Bullet Item 1\n- Bullet Item 2')} className="px-2.5 py-1 bg-white/5 hover:bg-white/10 text-xs rounded font-semibold border border-white/5">List</button>
                    </div>
                    ...
                    <Card variant="elevated" className="flex flex-col p-6 min-h-[500px]">
                        ...
                        <div className="flex items-center justify-between mb-3 border-b border-white/5 pb-2">
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${previewTab === 'visual' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-gray-400 hover:text-white'}`}
                                >
                                    <Eye size={14} />
                                    Visual Preview
                                </button>
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${previewTab === 'html' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'text-gray-400 hover:text-white'}`}
                                >
                                    <Code size={14} />
                                    HTML Code Output
                                </button>
                        ...
                        {previewTab === 'visual' ? (
                            <div 
                                ...
                                className="flex-grow p-4 bg-[#090d16] border border-white/5 rounded-xl overflow-y-auto max-h-[400px] text-gray-300 leading-relaxed font-sans"
                            />
                        ) : (
                            <textarea
                                readOnly
